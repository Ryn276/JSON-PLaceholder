let usersData = []; // Menyimpan data pengguna secara lokal

async function fetchUsers() {
    try {
        const response = await fetch("https://jsonplaceholder.typicode.com/users");
        const users = await response.json();
        usersData = users; // Simpan data dari API di variabel lokal
...
function displayUsers(users) {
    ...
    const allUsers = [...users, ...localUsers]; // Gabungkan dengan data dari API
...
// Fungsi untuk menghapus pengguna dari tabel dan dari localStorage
function deleteUser(userId) {
    // Hapus pengguna dari usersData
    usersData = usersData.filter(user => user.id !== userId);
    
    // Hapus pengguna dari localStorage
    let localUsers = JSON.parse(localStorage.getItem("users")) || [];
    localUsers = localUsers.filter(user => user.id !== userId);
    localStorage.setItem("users", JSON.stringify(localUsers));

    displayUsers(usersData); // Tampilkan pengguna yang tersisa
}
